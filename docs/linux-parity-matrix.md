...
| Distro package install tests | `partial` | [nix/tests-distro.nix](/Users/jess/git/cmux/nix/tests-distro.nix:1), [.github/workflows/test-distro.yml](/Users/jess/git/cmux/.github/workflows/test-distro.yml:1) | Ubuntu 24.04, Debian 12, and Fedora 42 are covered directly; Rocky 10 is now branch-wired through a dedicated `rpmRocky` asset while the checked-in manifest still leaves Rocky 9 as the default proxy; current KVM tests execute `.sandboxed` VM runs on push/manual, not as PR-gating | keep Fedora 42 green, record the first Rocky 10 run, retire Rocky 9, and decide how fresh-install proof should gate release confidence |
