...
| Socket/API control plane | `partial` | [socket.zig](/Users/jess/git/cmux/cmux-linux/src/socket.zig:1), [app.zig](/Users/jess/git/cmux/cmux-linux/src/app.zig:1) | all Linux targets, but several high-value verbs and host callbacks are still stubbed | implement and validate `surface.send_text`, `surface.read_text`, `pane.break`, `pane.join`, `surface.move`, `surface.reorder`, and Linux action/clipboard callbacks |
