...
| Distro package install tests | `partial` | [nix/tests-distro.nix](/Users/jess/git/cmux/nix/tests-distro.nix:1), [.github/workflows/test-distro.yml](/Users/jess/git/cmux/.github/workflows/test-distro.yml:1) | Ubuntu 24.04, Debian 12, and Fedora 42 are covered directly; Rocky 9 remains the checked-in RPM-path proxy while Rocky 10 terminal-first proof waits on a distinct no-WebKit RPM artifact path; upstream `nix-vm-test` image support is no longer the blocker | keep Fedora 42 green, record first Rocky 10 terminal-first proof, retire Rocky 9 as the proxy, and decide how fresh-install proof should gate release confidence |
